...
const CategoryDeatailsPopular = () => {
  const { data, loading, error } = useContext(DataContext);

  if (loading) return <p>Loading...</p>;
  if (error) return <p>Error: {error}</p>;
  const popularItems =
    data && data.filter((item) => item.category === "popular");

  return (
    <div>
      <div className="shadow-xl">
        <h1 className="text-[32px] font-bold py-4">Popular Blogs</h1>
...
export default CategoryDeatailsPopular;
